...
extract_translations: ## extract strings to be translated, outputting .po files
	...
	if [ -f "$(EXTRACTED_DJANGOJS)" ]; then msgcat $(EXTRACTED_DJANGO) $(EXTRACTED_DJANGOJS) -o $(EXTRACTED_TEXT); else mv $(EXTRACTED_DJANGO) $(EXTRACTED_TEXT); fi
...
compile_translations: ## compile translation files, outputting .mo files for each supported language
	...
	python manage.py compilejsi18n --namespace EOCJournalXBlockI18N --output $(JS_TARGET)
